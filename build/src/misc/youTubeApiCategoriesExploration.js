...
const scrape = (dataSource, log, _api) => __awaiter(void 0, void 0, void 0, function* () {
    const youtubeIdsWithoutMetadata = yield dataSource
        .createQueryBuilder()
...
const createDataSource = (projectRootDir) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    const ds = new typeorm_1.DataSource(Object.assign(Object.assign({ type: 'postgres' }, dbConfig), { username: dbUser, synchronize: false, entities: entities_1.default, namingStrategy: new typeorm_naming_strategies_1.SnakeNamingStrategy(), logging: true, maxQueryExecutionTime: 200, logger: new databaseLogger_1.default(console.log) }));
    yield ds.initialize();
    return ds;
});
const main = () => __awaiter(void 0, void 0, void 0, function* () {
    ...
    const compareCategoriesOfTwoRegions = (regionA, regionB) => __awaiter(void 0, void 0, void 0, function* () {
        ...
        if (mapA.size !== mapB.size) {
            console.log(`Categories of ${regionA} and ${regionB} differ in size,`, `region ${regionA} has ${mapA.size} categories,`, `while region ${regionB} has ${mapB.size} categories.`);
        }
        for (const [id, categoryA] of mapA) {
            const categoryB = mapB.get(id);
            if (!categoryB) {
                console.log(`Category ${categoryA.snippet.title} (${id})`, `is only available in region ${regionA},`, `but not in region ${regionB}.`);
            }
        }
        for (const [id, categoryB] of mapB) {
            const categoryA = mapA.get(id);
            if (!categoryA) {
                console.log(`Category ${categoryB.snippet.title} (${id})`, `is only available in region ${regionB},`, `but not in region ${regionA}.`);
            }
        }
        for (const [id, categoryA] of mapA) {
            const categoryB = mapB.get(id);
            ...
            if (categoryA.snippet.title !== categoryB.snippet.title) {
                console.log(`Category ${categoryA.snippet.title} (${id})`, `is called ${categoryB.snippet.title} in region ${regionB}.`);
            }
        }
    });
    ...
    if (cmd === 'compare') {
        ...
        const log = (0, logger_1.createDefaultLogger)((0, fs_1.createWriteStream)('scrape.log'));
        yield scrape(dataSource, log, api);
    }
    else {
